...
> **你们只需要关心 `backend/routers/` 下的 `.py` 文件和 `backend/schemas.py`。**
> `main.py`、`database.py`、`models.py`、`utils.py` 已经写好了，一般不需要动。
> `routers/chat.py` 是楚雪负责的 AI 模块，**不要碰**。

---

## 🟢 当前现状（你们拿到的是什么）

后端使用 Python + FastAPI + SQLite，已经能跑。运行方式：
```bash
cd backend
...
│   ├── auth.py          认证接口（✅ 已完成，需验证 + 小优化）
│   ├── emotion.py       情绪记录接口（✅ 基本完成，需增强）
│   └── user.py          用户档案接口（✅ 基本完成，需增强）
│
...
## 📋 任务总表：调通现有接口（核心目标）AI分配的，你们自行协商。自己记住完成了什么部分，给老师的文档要填具体的分工技术部分

目前的后端代码已具备基本框架，但仍有一些逻辑漏洞或对接细节需要你们**调通并验证**。

### 后端同学 A：负责「用户与认证模块」调通
**涉及文件：** `routers/auth.py` + `routers/user.py`

你的任务是确保用户能正常注册、登录，并获取到个人信息：
...
### 后端同学 B：负责「情绪记录与语料库模块」调通
**涉及文件：** `routers/emotion.py` + `routers/common.py`

你的任务是确保情绪数据能存得进去，并且首页能随机展示语录：
...
1. **启动服务**：在 `backend` 目录下运行 `python main.py`。
...
1. **不要碰** `routers/chat.py` ——这是楚雪的 AI 模块
2. 所有接口返回格式统一用 `success_resp(data={...})` 或 `error_resp(msg="...")`（在 `utils.py` 里定义好了）
3. 前端传参方式：POST 请求用 JSON body，GET 请求用 query 参数（如 `?user_id=1&year=2026`）
4. 如果新增了字段想改表结构（`models.py`），最简单的方法是**删掉 `emochat.db`** 再重新运行 `python main.py`，它会自动重建新库
5. 写完后在 `http://127.0.0.1:8000/docs` 先自己测一遍，确保 200 返回正常
