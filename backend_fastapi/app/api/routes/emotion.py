...
EMOTION_CONFIG = {
    ...
    "reasonTags": ["学习", "人际", "工作", "家庭", "情感", "天气", "生理期", "健康", "金钱", "未来", "睡眠", "饮食", "社交媒体", "娱乐休闲", "自我成长", "宠物", "交通出行", "外貌形象", "居住环境", "新闻事件"],
    "emotionRules": {
...
def get_emotion_tags():
    return success_resp(data=EMOTION_CONFIG)


@router.post("/record")
def create_record(req: schemas.RecordRequest, db: Session = Depends(get_db)):
    payload = emotion_service.create_record(db, req)
    return success_resp(data=payload)


@router.get("/calendar")
def get_calendar(user_id: int, year: int, month: int, db: Session = Depends(get_db)):
    payload = emotion_service.get_calendar(db, user_id=user_id, year=year, month=month)
    return success_resp(data=payload)


@router.get("/insights")
def get_emotion_insights(user_id: int, year: int, month: int, db: Session = Depends(get_db)):
    payload = emotion_service.get_insights(db, user_id=user_id, year=year, month=month)
    return success_resp(data=payload)


@router.get("/detail")
def get_emotion_detail(user_id: int, date_str: str, db: Session = Depends(get_db)):
    ok, payload = emotion_service.get_detail(db, user_id=user_id, date_str=date_str)
    if not ok:
        return payload
    return success_resp(data=payload)
...
def get_emotion_history(
    ...
    payload = emotion_service.get_history(
        db,
        user_id=user_id,
        page=page,
        page_size=page_size,
